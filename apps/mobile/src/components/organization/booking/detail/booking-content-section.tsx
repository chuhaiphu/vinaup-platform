...
export function BookingContentSection() {
  const [isExpanded, setIsExpanded] = useState(true);
  const contentModalRef = useRef<SlideSheetRef>(null);

  const { booking, canEdit, isUpdatingBooking, handleUpdateBooking } = useBookingDetailContext();
  ...
  const handleConfirmUpdate = (value: string, onSuccessCallback?: () => void) => {
    handleUpdateBooking({ content: value }, () => onSuccessCallback?.());
  };
        ...
        <View style={[styles.section, !isExpanded && styles.sectionCollapsed]}>
          {isExpanded && (
            <View style={styles.sectionContent}>
              <Text style={booking.content ? styles.contentText : styles.placeholderText}>
                {booking.content || '...'}
              </Text>
            </View>
          )}
