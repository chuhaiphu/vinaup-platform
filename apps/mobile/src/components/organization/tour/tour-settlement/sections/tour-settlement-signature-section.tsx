...
import { TourSettlementCancelLogModal } from '@/components/organization/tour/tour-settlement/modals/tour-settlement-cancel-log-modal/tour-settlement-cancel-log-modal';
...
export interface TourSettlementSignatureRef {
  refresh: () => void;
}

interface TourSettlementSignatureProps {
  onOpenSignatureInfoPopover?: () => void;
  ref?: React.Ref<TourSettlementSignatureRef>;
}

export default function TourSettlementSignature({
  onOpenSignatureInfoPopover,
  ref,
}: TourSettlementSignatureProps) {
  ...
  const {
    tourSettlement,
    signatures: tourSettlementSignatures,
  ...
  useEffect(() => {
    ...
    fetchSignatures();
    fetchOrganizationMembers();
  }, [settlementId, fetchSignatures, fetchOrganizationMembers]);
  ...
  const sender = tourSettlementSignatures?.find((s) => s.signatureRole === 'SENDER');
  const receivers = tourSettlementSignatures?.filter((s) => s.signatureRole === 'RECEIVER');
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          {isTourSettlementPrivate && <Text style={styles.statusLabel}>Chỉ bạn nhìn thấy</Text>}
          <PressableOpacity
        ...
        <View style={styles.signaturesGroup}>
          <View style={styles.signatureColumnLeft}>
            ...
            <SignatureEntity
              ...
              role="SENDER"
              isLoading={isLoading || isSigningTourSettlement || isCancelingTourSettlement}
              alignment="left"
      ...
      <ConfirmSlideSheet
        ...
        onConfirmPress={handleConfirmSignTourSettlement}
      />
      <ConfirmSlideSheet
        ...
        onCloseCompleted={() => setSelectedSignatureId(null)}
        onConfirmPress={handleConfirmCancelTourSettlement}
      />
      <TourSettlementCancelLogModal modalRef={cancelLogModalRef} />
    </View>
  );
}
