...
sequenceDiagram
    ...
    alt Precise
        H->>OS: getCurrentPositionAsync + reverseGeocodeAsync
        OS-->>S: coordinate code + address
    else Not precise
...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `location`               | The `Địa điểm` input, pre-filled from `reverseGeocodeAsync` via [`generateLocaleAddress`](../../src/utils/generator/string-generator/generate-locale-address.ts) | Yes — it is the person's own label                                                             |
