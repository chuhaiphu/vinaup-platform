...
The Notifier pattern is a **Facade over two Strategy families**. Business code gets one object that sends
...
src/notifier/
  ...
  notifier.interface.ts     MailIntent, PhoneIntent, SendOutcome
...
One file per driver, named for the transport it speaks. `notifier.interface.ts` stays inside the module
because nothing outside consumes those types.
...
export type PhoneIntent =
  | { kind: 'SIGN_UP_OTP'; code: string }
  | { kind: 'SIGN_IN_OTP'; code: string }
...
`this.notifier.sendSignUpOtp(phone, code)` — see [Using it](#calling-the-facade).
...
@Module({
  providers: [
    ...
    SmtpMailService, LogMailService,
    ZnsPhoneService, SmsPhoneService, FallbackPhoneService, LogPhoneService,
...
`inject` is the **argument list of `useFactory`**: entry *n* becomes parameter *n*.
...
`case`, and TypeScript fails the build — *not all code paths return a value*.
...
A `useFactory` body is only *defined* in Phase 1 and *called* in Phase 2, once config exists. → [Factory Pattern — import time vs instantiation time](FACTORY-PATTERN.md#import-time-vs-instantiation-time)
...
export class NotifierService {
  ...
  sendSignUpOtp(phone: string, code: string): void {
  ...
  private dispatch(sending: Promise<SendOutcome>): void {
    void sending
      .then((outcome) => { if (!outcome.delivered) this.logger.error(outcome.reason); })
      .catch((error: unknown) => this.logger.error(error));
  }
}
```

`sendSignUpOtp` calls `this.phone`; `sendPasswordResetOtpToEmail` calls `this.mail`. Each method names
...
A driver returns a `SendOutcome` rather than throwing, there are **two readers that read the outcome:**:
...
A log driver satisfies the contract **without a production-ready-provider**: instead of building a provider request, it
writes the destination and the intent through `Logger` and always reports success. That is what lets every flow run end to end with no provider
...
> **`NotifierModule` refuses to boot a log driver in production.** It asserts at startup that
> `NODE_ENV !== 'production'` or the driver is a network one.
...
```ts
this.notifier.sendSignUpOtp(phone, code); // no await, no catch, no transport named
```

| Facade method | Contract | Flow |
| --- | --- | --- |
| `sendSignUpOtp(phone, code)` | `PhoneService` | [Sign-Up](../architecture/authen/SIGN-UP.md) — step 1 |
| `sendSignInOtp(phone, code)` | `PhoneService` | [Local Sign-In](../architecture/authen/LOCAL-SIGN-IN.md#mode-2--otp) — OTP mode |
| `sendEmailVerificationOtp(email, code)` | `MailService` | [Link Email](../architecture/authen/LINK-EMAIL.md) — step 1 |
| `sendPasswordResetLink(email, resetUrl)` | `MailService` | [Password Reset — Email Link](../architecture/authen/PASSWORD-RESET-EMAIL-LINK.md) |
...
| `sendPasswordResetOtpToPhone(phone, code)` | `PhoneService` | password reset to a phone number |
...
2. **Implement `send` — turn the `intent` into that provider's request body.** `send` receives an intent
   ...
   | **app-messaging** | an account inside that messaging app | the **id of a template the provider approved beforehand**, plus the parameters that template declares | the provider owns the wording and rejects free text, so the driver only chooses *which* approved template fits this `kind` and fills its blanks |
