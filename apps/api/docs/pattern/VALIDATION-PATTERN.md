...
Validation is **schema-first**. Every request shape is a **Zod schema** declared once in the shared `@vinaup-platform/validation` package. The api app enforces it with a global `ZodValidationPipe` (nestjs-zod); the mobile app reuses the *same* schema object in its forms.
...
| Job | **Checks** data at the boundary (parse / reject / coerce) | **Annotates** variables, parameters, return shapes |
| Lives in | `packages/validation/src/zod-schemas/` | `packages/validation/src/interfaces/` |

We write the schema once; the interface is *derived* from it with `z.infer`.
...
1. **Global locale** — the package barrel installs Zod's Vietnamese locale once; every schema produces Vietnamese *default* messages:
...
The pipe runs **after** the guards (see [GUARD-PATTERN.md](GUARD-PATTERN.md)) and immediately before the **handler** (the controller method bound to the route). The request is authenticated/authorized first, *then* its body is parsed:
...
Pipes             — validate / transform the handler args    ◄── ZodValidationPipe
  │                  schema.parse() → coerce & strip, or 400 on failure
  ▼
...
export class EntityController {
  ...
  create(@Body() body: CreateEntityRequest) {  // handler bound to that route
...
1. **Nest router** matches the route, then reads the *class* annotating `@Body()` — `CreateEntityRequest`. It reads the class, not the type: a `z.infer` type alias is erased at build time, so the validation rules must travel on something that survives at runtime — the class.
...
A **pipe** in NestJS is a class that sits between the moment a request is **matched to a route** and the moment its **handler runs**. It receives the incoming value (a body, a query, a param) and may **transform** it (coerce, strip) or **reject** it (throw → the handler never runs). Validation is exactly this job, so it lives in a pipe.
...
The schema answers *"is the body well-formed?"*. The service/guard answers *"is it allowed and consistent with current state?"*. 

- Never put a DB lookup in a schema
- Never re-check shape in a service
- A *pure* cross-field rule (no I/O, e.g. `endDate ≥ startDate`) belongs in a schema `.refine()`; a cross-field rule that needs the DB or current state goes in the service/guard
...
A field's shape is governed by **two** declarations: the **Prisma column** (can the DB store NULL?) and the **Zod rule** (what the client may send). The TypeScript type is *not* a third — it is `z.infer` of the rule, derived automatically and used only to type the input in the **service**:
...
The rule decides what the client may send; the type is inferred from it (this is the only Zod here — Prisma is shown as a comment for context):
...
|---------------|-------------------|-------------------------------|------------------------|
| *(none)*      | will be validated | ❌ reject 400                 | ❌ reject 400          |
...
|---------------|---------------------|----------------|------------------------|
| *(none)*      | `field: T`          | `T` (NOT NULL) | required, non-nullable |
...
Each request shape is written **once** as a Zod schema; the `ZodValidationPipe` parses every incoming request against **that schema** at the boundary — before the controller method runs. So by the time **the handler** executes, Zod has already checked every field, coerced it to its real type (the query string `"10"` becomes the number `10`), and stripped or rejected unknown keys. The handler therefore never sees malformed data and never re-validates: it can treat its input as well-formed and correctly typed.
...
5. **Coerce by source** — `z.coerce.*` for query/param, plain types for JSON body. → [Coding Convention §6](../CODING-CONVENTION.md)
6. **Messages are Vietnamese** — the package barrel installs `z.config(z.locales.vi())`; write a `{ error: '…' }` override only where the locale default reads poorly for the field. → [Vietnamese messages](#vietnamese-messages-locale--per-field-overrides)
7. Bridge into the API with `class <Action>Request extends createZodDto(<action>Schema)`; never restate rules in the controller.
8. Register `ZodValidationPipe` once globally; nothing else to wire.
9. Existence, ownership, state-machine, and DB-dependent cross-field rules go in the service or a guard; a pure cross-field rule (no I/O) uses a schema `.refine()`. → [Guard Pattern](GUARD-PATTERN.md)
10. Let Zod's issue path name the field; the global pipe and exception filter surface it. → [Exception Filter Pattern](EXCEPTION-FILTER-PATTERN.md)
