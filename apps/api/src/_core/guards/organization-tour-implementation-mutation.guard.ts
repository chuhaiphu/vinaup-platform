...
import { TourImplementationNotFoundException, TourNotFoundException } from 'src/_common/exceptions/tour.exception';
import { AuthenticatedRequest } from 'src/_common/interfaces/interface';
import { PrismaService } from 'src/prisma/prisma.service';

@Injectable()
export class OrganizationTourImplementationMutationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const tour = await this.prismaService.tour.findUnique({
      where: { id: tourImplementation.tourId },
