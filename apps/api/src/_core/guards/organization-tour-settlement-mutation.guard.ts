...
import { TourNotFoundException, TourSettlementNotFoundException } from 'src/_common/exceptions/tour.exception';
import { AuthenticatedRequest } from 'src/_common/interfaces/interface';
import { PrismaService } from 'src/prisma/prisma.service';

@Injectable()
export class OrganizationTourSettlementMutationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!tourSettlement) throw new TourSettlementNotFoundException();

    const tour = await this.prismaService.tour.findUnique({
      where: { id: tourSettlement.tourId },
